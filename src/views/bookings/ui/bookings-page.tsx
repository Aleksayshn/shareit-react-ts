...
import { Card, EmptyState, Field, LinkButton, PageShell, Select, Spinner } from "@/src/shared/ui";
...
function useBookingListUrlState() {
  ...
  const size = bookingPageSizeOptions.includes(requestedSize as (typeof bookingPageSizeOptions)[number])
    ? requestedSize
    : 10;
...
export function BookingsPage({ mode }: BookingsPageProps) {
  ...
  const title = mode === "mine" ? "Track your bookings" : "Review booking requests";
  const description =
    mode === "mine"
  ...
  const emptyTitle = mode === "mine" ? "No bookings found" : "No booking requests found";
  ...
  return (
    <PageShell
      actions={
        <div className="flex flex-wrap gap-3">
          <LinkButton href="/bookings" size="sm" variant={mode === "mine" ? "primary" : "ghost"}>
            My bookings
          </LinkButton>
          <LinkButton
            href="/bookings/owner"
    ...
    >
      ...
      {hasHydrated && !selectedUserId ? (
        <EmptyState
          description="Booking pages depend on the active user store because the API client injects `X-Sharer-User-Id` from the selected sharer."
